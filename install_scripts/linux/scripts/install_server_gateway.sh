...
apt-get install -y wget curl git ufw 1>/dev/null
...
info "Creating application paths ..." # ? change paths
mkdir -p /opt/sentinel/install
mkdir -p /opt/sentinel/application

info "Downloading sentinel gateway application ..." # TODO
# curl -sLO 'https://github.com/pasleto/sentinel/blob/main/install_scripts/linux/content/sentinel_gateway.zip' -o /opt/sentinel/install/sentinel_gateway.zip
# unzip -q /opt/sentinel/install/sentinel_gateway.zip # -d /opt/sentinel/install/sentinel_gateway

info "Installing sentinel gateway application ..." # TODO
# cp -r /opt/sentinel/install/sentinel_gateway/. /opt/sentinel/application/
# pushd /opt/sentinel/application/
# npm install # /usr/bin/npm
# popd
...
info "Creating sentinel gateway service ..." # ? - validate
...
case "$CLOUD_DEPLOY" in
  [Yy]*)
    if [ -d "/etc/letsencrypt/live/${SSL_DOMAIN_DIR}" ]; then # ? - check if directory /etc/letsencrypt/live/${SSL_DOMAIN_DIR}/ exist 
      if [ ! -d "/opt/sentinel/application/certificates-orig" ]; then # ? - backup existing localhost certificates - skip if certificates-orig directory already exist -- one time run
