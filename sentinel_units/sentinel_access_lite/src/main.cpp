...
String default_lock_id =  "01"; // because this module has only one reader, but to make it more universal
...
void initDatabaseFile(const char *database_path, sqlite3 *database_instance) { // TODO
  ...
  if (rc_config_silent_mode != SQLITE_OK) {
    ...
    return;
            ...
            // TODO - for test only
            String cards_insert_dummy_query_01 = "INSERT INTO cards VALUES ('04C2B401', NULL)";
            int cards_insert_dummy_01_rc = dbExec(database_instance, cards_insert_dummy_query_01.c_str());
            if (cards_insert_dummy_01_rc != SQLITE_OK) {
              sqlite3_close(database_instance);
              return;
            }
            String cards_insert_dummy_query_02 = "INSERT INTO cards VALUES ('A4DCCC1D', NULL)";
            int cards_insert_dummy_02_rc = dbExec(database_instance, cards_insert_dummy_query_02.c_str());
            if (cards_insert_dummy_02_rc != SQLITE_OK) {
              sqlite3_close(database_instance);
              return;
            }
            String cards_insert_dummy_query_03 = "INSERT INTO cards VALUES ('44C20C45', '364A9D')";
            int cards_insert_dummy_03_rc = dbExec(database_instance, cards_insert_dummy_query_03.c_str());
            if (cards_insert_dummy_03_rc != SQLITE_OK) {
              sqlite3_close(database_instance);
              return;
            }
...
void wiegandReaderStateChanged(bool plugged, const char* reader_ID) { // TODO - pull-down resistors on wiegand data pins
  if (plugged) {
    wg_connected = true;
    digitalWrite(ESP_WG_GREEN_LED, HIGH); // OFF
    digitalWrite(ESP_WG_RED_LED, HIGH); // OFF
...
void setup() {
  ...
  digitalWrite(ESP_RXD_LED, LOW); // ON
  delay(500);
  digitalWrite(ESP_TXD_LED, LOW); // ON // ? maybe blink led on door state
  // ----------------------------------------------------------------------------------------------------------------
  delay(27500);
  setupWiegandReader();
  // digitalWrite(ESP_WG_GREEN_LED, HIGH); // OFF
...
void loop() {
  // TODO - pull-down resistors on wiegand data pins - check functionality with pull-downs || remove wg_connected feature
  if (!wg_connected || (!wg_halt && wg_connected)) wiegandReaderHandler();
  if (wg_halt && wg_connected) wiegandReaderStateHandler();
  // if (!wg_halt) wiegandReaderHandler();
  // if (wg_halt) wiegandReaderStateHandler();
