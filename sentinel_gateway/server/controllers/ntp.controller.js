...
import { ntpLoggerHandler } from './logger.controller.js';
...
class NTPPacket {
	...
	static parse(data) {
		if (data.length < 48) {
			const err = new Error('Invalid NTP Package');
			err.data = data;
			throw err;
		}
		...
		// Timestamps where the 4 first bytes are the int part and the 4 last are the frac part
	...
	bufferize(packet) {
		...
		// Originate timestamp
		writeInMillis(buffer, 24, packet.originateTimestamp, this.mode !== 4); // Don't add NTP_DELTA if the packet is server mode
		// RX Timestamp
...
class NTPServer extends EventEmitter {
	...
	handle() {
		this.on('request', (message, response) => {
	...
	send(rinfo, message, callback) {
		if (message instanceof NTPPacket) {
			const sendPackage = new NTPPacket(4).bufferize(message);
			this.socket.send(sendPackage, rinfo.port, rinfo.address, callback);
			// ntpLoggerHandler(rinfo.family, rinfo.address, 'SUCCESS', '');
			// ntpLoggerHandler(rinfo.family, rinfo.address, 'SUCCESS', `- ${JSON.stringify(message)}`);
			return this;
		} else {
			// ntpLoggerHandler(rinfo.family, rinfo.address, 'ERROR', '- Invalid Response Packet');
			ntpLoggerHandler(rinfo.family, rinfo.address, 'ERROR', `- Invalid Response Packet - ${JSON.stringify(message)}`);
			return;
		}
	}

	listen(port, address) {
		this.socket.bind(port || 123, address);
		return this;
	}
	...
	onListening(handler) {
		this.socket.on('listening', handler);
		return this;
	}
	...
	parse(message, rinfo) {
		...
		// ntpLoggerHandler(rinfo.family, rinfo.address, 'SUCCESS', '');
		ntpLoggerHandler(rinfo.family, rinfo.address, 'SUCCESS', `- ${JSON.stringify(packet)}`);
		return;
		} catch (error) {
			ntpLoggerHandler(rinfo.family, rinfo.address, 'ERROR', `- ${error.message} - ${JSON.stringify(error.data)}`);
			return;
		}
	}
}
