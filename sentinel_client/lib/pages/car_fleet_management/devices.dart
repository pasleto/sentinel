...
import 'package:sentinel_client/models/cfDevice.model.dart';
...
class CarFleetManagementDevicesPage extends StatefulWidget {
  const CarFleetManagementDevicesPage({super.key});

  @override
  State<CarFleetManagementDevicesPage> createState() => _CarFleetManagementDevicesPageState();
}

class _CarFleetManagementDevicesPageState extends State<CarFleetManagementDevicesPage> with PageMixin {
  bool _pageInScope = false;

  bool _initialized = false;
  bool _fetchingFailed = false;
  List<CfDevice> _devices = []; // TODO - list of cars
  ...
  void initState() {
    ...
    print('carfleet-management-devices initState');
  ...
  @override
  void dispose() {
    print('carfleet-management-devices dispose');
  ...
  Widget build(BuildContext context) {
    return VisibilityDetector(
      key: const Key('carfleet-management-devices'),
      onVisibilityChanged: (visibilityInfo) {
        if (visibilityInfo.visibleFraction == 1) {
          if (!_pageInScope) {
            setState(() => _pageInScope = !_pageInScope);

            print('carfleet-management-devices loading');

            SocketService.socket.emitWithAck('cf-device-get-all', {}, ack: (response) {
              if (response['status'] == 'OK') {
                Iterable incoming = response['data']['devices'];
                setState(() {
                  _devices = List<CfDevice>.from(incoming.map((model) => CfDevice.fromJson(model)));
                  print(incoming); // TODO
                  _initialized = true;
        ...
        if (visibilityInfo.visibleFraction == 0) {
          if (_pageInScope && mounted) {
            ...
            print('carfleet-management-devices unloading');
          }
        }
      },
      child: ScaffoldPage(
        header: PageHeader(
          title: const Text('Car Fleet - Devices', style: TextStyle(fontSize: 18.0, fontWeight: FontWeight.w600)),
                            ...
                            children: [
                              Expanded(
                                child: TextBox(
                                  ...
                                  placeholder: ' Identifier, MAC Address, Description',
